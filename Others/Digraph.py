...
def sap(g, aList, bList):
    ...
    if common_values:
        ...
        while not Q.empty():
            v, source, distance = Q.get()

            if distance + 1 >= sapLength:
                break

            for w in g.adj[v]:
                if source == 'a':
                    if (w, source) not in visited:
                        Q.put((w, source, distance + 1))
                        visited.add((w, source))
                        if (w, 'b') in visited:
                            temp = distance + 1 + bfs(w, g, bList)
                            if temp < sapLength:
                                ancestor = w
                                sapLength = temp
...
def bfs(node, g, targetList):
    ...
    while not Q.empty():
        ...
        if next == node:
            return total
        for w in g.adj[next]:
            if (w, total + 1) not in visited:
                visited.add((w, total + 1))
                Q.put((w, total + 1))
