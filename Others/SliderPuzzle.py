...
class Board:
    ...
    def neighbors(self):
        def createNeighbor(tiles, row, col):            
            tiles[self.rowZero][self.colZero], tiles[row][col] = tiles[row][col], 0
            neighbor = Board(self.tiles)
            tiles[self.rowZero][self.colZero], tiles[row][col] = 0, tiles[self.rowZero][self.colZero]
            return neighbor

        neighborList = []
        if self.rowZero>0: neighborList.append(createNeighbor(self.tiles, self.rowZero-1, self.colZero))
        if self.rowZero<self.dimension()-1: neighborList.append(createNeighbor(self.tiles, self.rowZero+1, self.colZero))
        if self.colZero>0: neighborList.append(createNeighbor(self.tiles, self.rowZero, self.colZero-1))
        if self.colZero<self.dimension()-1: neighborList.append(createNeighbor(self.tiles, self.rowZero, self.colZero+1))
...
def solveManhattan(initialBoard):
    ...
    pq.put((0 + initialBoard.manhattan(), initialBoard, 0, None, [initialBoard]))

    while True:
        _, now, count, past, result = pq.get()

        if now.isGoal():
            return result
        else:
            neighbors = now.neighbors()
            for neighbor in neighbors:
                assert(isinstance(neighbor, Board))
                if past is None or neighbor != past:
                    pq.put((count + 1 + neighbor.manhattan(), neighbor, count + 1, now, result + [neighbor]))
