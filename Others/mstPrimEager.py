...
class WUGraph:
    def __init__(self, V): # Constructor
        ...
        self.adj = [[] for _ in range(V)]   # adj[v] is a list of vertices adjacent to v 간선 객체가 들어감
        self.edges = []
...
def mstPrimEager(g):
    def include(i):
        ...
        for e in g.adj[i]:
            ...
            if not pq.contains(x):
                pq.insert(x, e)
            elif pq.keyOf(x).weight > e.weight:
                pq.decreaseKey(x, e)
    ...
    while not pq.isEmpty() and len(result) < g.V - 1:
        e, w = pq.delMin()
        if included[e.v] and included[e.w]:
            continue
        result.append(e)
        include(w)
        weightSum += e.weight
